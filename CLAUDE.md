...
pnpm run db:setup         # Run migrations and seeds (works with remote or local)
pnpm run db:generate      # Generate Drizzle migrations from schema changes
pnpm run db:push          # Push migrations via Supabase CLI
pnpm run db:local         # Start local Supabase (includes PostgreSQL + PostGIS + Studio)
...
The project supports **multiple database setups**:

1. **Supabase Remote** (Production) - Connected to Supabase project (wwiwmsgedqeepcdtnhon) via connection pooler
2. **Supabase Local** (Development) - Local PostgreSQL via Supabase CLI (`pnpm run db:local`)
3. **Docker Compose** (Development) - Local PostgreSQL with Docker (see `database/docker-compose.yml`)
...
2. Apply migrations and seeds: `pnpm run db:setup`
...
**Database utilities** (in `database/scripts/`):
...
- **`database/scripts/verify-db.ts`** - Verify database setup
- **`database/scripts/clean-db.ts`** - Clean all tables
- **`database/scripts/check-rls.ts`** - Check RLS status
- **`database/scripts/check-permissions.ts`** - Check table permissions
