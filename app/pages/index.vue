...
const { category, openNow, nearMe, searchResults: locations, status, hasMore, loadMore, hasSearchParams } = useSearch()

// Fetch categories for filter display
const { data: categoriesData } = useFetch('/api/categories')

// Event handlers for QuickFilters only
function handleCategoryUpdate(value: string | undefined) { category.value = value }
function handleOpenNowUpdate(value: boolean) { openNow.value = value }
function handleNearMeUpdate(value: boolean) { nearMe.value = value }
</script>

<template>
  <div>
    <QuickFilters f-mt-2xs f-mb-md :category='category' :open-now='openNow' :near-me='nearMe' :categories='categoriesData' @update:category='handleCategoryUpdate' @update:open-now='handleOpenNowUpdate' @update:near-me='handleNearMeUpdate' />
