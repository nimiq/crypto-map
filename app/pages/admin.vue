...
type LocationWithStatus = NonNullable<typeof data.value>['locations'][number] & {
  hoursStatus?: {
          ...
          <table w-full text='f-xs left' border-collapse>
            ...
            <tbody>
              <tr v-for='location in locations' :key='location.uuid' bg='white hover:neutral-50' border-b='1 neutral-200' group transition-colors>
                ...
                <td px-8 py-8>
                  <template v-if='location.hoursStatus'>
                    <div flex='~ items-center gap-4'>
                      <span :class='variantClasses[location.hoursStatus.status.variant as keyof typeof variantClasses]' font-medium px-4 py-1 rounded-3 whitespace-nowrap text='10px'>
                        {{ location.hoursStatus.status.messageKey }}
                      </span>
                        ...
                        <TooltipPortal>
                          <TooltipContent border='1 neutral-200' px-12 py-8 rounded-8 bg-white max-w-400 shadow-lg z-50>
                            <div text='f-xs neutral-700' font-mono>
                              <div v-for='(line, index) in location.hoursStatus.formattedHours' :key='index'>
                                {{ line }}
                              </div>
                            </div>
                ...
                <td px-8 py-8>
                  <div flex='~ col gap-2 items-start'>
                    <button type='button' text='f-2xs neutral-600 hover:neutral-900 left' bg='neutral-50 hover:neutral-100' font-mono px-6 py-2 rounded-4 cursor-pointer whitespace-nowrap transition-colors @click='copyToClipboard(location.uuid, 'UUID copied')'>
                      {{ location.uuid }}
                    </button>
                    <button type='button' text='f-2xs neutral-600 hover:neutral-900 left' bg='neutral-50 hover:neutral-100' font-mono px-6 py-2 rounded-4 cursor-pointer whitespace-nowrap transition-colors @click='copyToClipboard(location.gmapsPlaceId, 'Place ID copied')'>
