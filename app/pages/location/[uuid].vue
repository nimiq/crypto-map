...
const { status: openingStatus }
  = location.value?.openingHours && location.value?.timezone
...
const weeklyHours = computed(() => {
  ...
  const { hours: rotatedHours, separatorAfterIndex } = rotateWeeklyHoursToToday(
    hours,
    location.value.timezone,
    locale.value,
  )
...
const statusColorClass = computed(() => {
  ...
  switch (openingStatus.value.variant) {
    case 'success':
      return 'bg-green-400 text-green-1100 outline-green-500'
    case 'warning':
      return 'bg-orange-400 text-orange-1100 outline-orange-500'
    case 'neutral':
      return 'bg-neutral-400 text-neutral-800 outline-neutral-500'
    default:
      return 'bg-neutral-200 text-neutral-700'
  }
})

const websiteInfo = computed<WebsiteInfo | null>(() =>
  location.value?.website ? parseWebsiteUrl(location.value.website) : null,
)
  ...
  <div v-else-if="location" flex="~ col" f-mt-md>
    ...
    <div relative>
      <button
        ...
        bg="white/25"
        outline="1.5 white/8 offset--1.5 ~"
        shadow-md
        rounded-full
        size-40
        transition-colors
    ...
    <div flex="~ col gap-12" f-mt-md>
      <div flex="~ col gap-8">
        <h1
          text="neutral-900 f-lg"
          lh-tight
          font-bold
          m-0
          :style="{ viewTransitionName: `location-name-${location.uuid}` }"
        >
          {{ location.name }}
        </h1>
    ...
    <div flex="~ items-center gap-20 wrap" f-mt-md>
      ...
      <NuxtLink
        v-if="websiteInfo"
        :to="websiteInfo.url"
        :nq-arrow="websiteInfo.hasArrow ? '' : undefined"
        external
        font-medium
        bg-transparent
        after:op-90
    ...
    <!-- Categories -->
    <div
      v-if="location.categories && location.categories.length > 0"
      flex="~ col gap-8"
      f-mt-lg
    >
      ...
      <div flex="~ wrap gap-8">
        <NuxtLink
          ...
          py-3
          rounded-full
          outline="1.5 neutral/2 dark:white/4 ~ offset--1.5"
          bg="neutral-300 hocus:neutral-400"
          un-text="neutral-800 f-xs"
          no-underline
          transition-colors
        >
      ...
      <div flex="~ col gap-4" text="neutral-700 f-xs">
        <template v-for="(entry, i) in weeklyHours.entries" :key="i">
          <div flex="~ justify-between gap-16">
            ...
            <span v-if="entry.isClosed" text-neutral-700 italic>{{
              t("hours.closed")
            }}</span>
            <span v-else flex="~ gap-4">
              <template
                v-for="(range, rangeIdx) in entry.timeRanges"
                :key="rangeIdx"
              >
                <span v-if="rangeIdx > 0">, </span>
                <NuxtTime
                  :datetime="`2024-01-01T${range.start}:00`"
                  hour="2-digit"
                  minute="2-digit"
                />
                <span> - </span>
                <NuxtTime
                  :datetime="`2024-01-01T${range.end}:00`"
                  hour="2-digit"
                  minute="2-digit"
                />
              </template>
          ...
          <hr
            v-if="weeklyHours.separatorAfterIndex === i"
