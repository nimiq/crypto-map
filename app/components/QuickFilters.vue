...
interface Emits {
  (e: 'removeCategory', categoryId: string): void
}
...
<template>
  <div translate-x="[calc(-1*var(--f-px))]" w-screen>
    <div flex="~ gap-8" of-x-auto nq-hide-scrollbar px="$f-px">
      <button
        v-for="catId in displayCategories"
        :key="catId"
        outline="~ 1.5 offset--1.5 white/10"
      ...
      >
        <Icon :name="getCategoryIcon(catId)" text-white size-16 />
        <span text="f-xs white" font-medium whitespace-nowrap>{{
          formatCategoryLabel(catId)
        }}</span>
        <Icon name="i-tabler:x" text-white op-70 size-16 />
      </button>
      ...
      >
        <ToggleGroupItem
          value="openNow"
          bg="neutral-0 reka-on:blue"
          outline="~ 1.5 offset--1.5 neutral/10 reka-on:white/10"
          flex="~ items-center gap-4"
          py-3
          rounded-full
          f-px-2xs
        >
          <Icon
            name="i-tabler:clock"
            text="neutral-700 reka-on:white"
            size-16
          />
          <span
            text="f-xs neutral-800 reka-on:white"
            font-medium
            whitespace-nowrap
          >{{ t("filters.openNow") }}</span>
        </ToggleGroupItem>
        <ToggleGroupItem
          value="nearMe"
          bg="neutral-0 reka-on:blue"
          ...
          f-px-2xs
        >
          <Icon name="i-tabler:walk" text="neutral-700 reka-on:white" size-16 />
          <span
            text="f-xs neutral-800 reka-on:white"
            font-medium
            whitespace-nowrap
          >{{ t("filters.nearMe") }}</span>
        </ToggleGroupItem>
      </ToggleGroupRoot>
    </div>
  </div>
