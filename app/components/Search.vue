    ...
    <ComboboxRoot v-model="selectedItem" :display-value="selectedItem">
      ...
      <ComboboxPortal>
        <ComboboxContent position="popper" side="bottom" :side-offset="4" w="$reka-combobox-trigger-width" outline="~ 1.5 neutral-200" rounded-b-8 bg-white max-h-256 shadow z-50 of-auto>
          <ComboboxViewport>
            <ComboboxItem :value="{ kind: 'query', query: searchQuery }" :key="searchQuery" flex="~ items-center gap-8" text="f-sm neutral-900" hocus:bg-neutral-50 font-medium py-10 outline-none w-full cursor-pointer transition-colors f-px-md>
