...
type SearchItem = | { kind: 'location', uuid: string, name: string } | { kind: 'query', query: string } | { kind: 'category', category: string, label: string }

interface Props { query?: string, category?: string, autocompleteResults?: SearchLocationResponse[] }
interface Emits { (e: 'update:query', value: string | undefined): void, (e: 'update:category', value: string | undefined): void, (e: 'navigate', uuid: string): void }
...
const searchDisplayValue = computed(() => shouldShowNearYou.value ? `${searchQuery.value} near you` : searchQuery.value)
...
const [DefineComboboxItemTemplate, Item] = createReusableTemplate<{ item: SearchItem, displayValue?: string, icon: string, color?: 'orange' | 'gold' | 'red' | 'purple' | 'green' | 'neutral', subline?: string }>()
  ...
  <DefineComboboxItemTemplate v-slot='{ item, displayValue, icon, color = 'neutral', subline }'>
    <ComboboxItem :value='getItemValue(item)' as-child>
      <button type='button' flex='~ items-center gap-16' px-16 py-12 text-left border-0 bg-transparent w-full cursor-pointer @click='handleItemClick(item)'>
        <div :class='{ 'outline-orange-500 bg-orange-400 text-orange-1100': color === 'orange', 'outline-gold-500 bg-gold-400 text-gold-1100': color === 'gold', 'outline-red-500 bg-red-400 text-red-1100': color === 'red', 'outline-purple-500 bg-purple-400 text-purple-1100': color === 'purple', 'outline-green-500 bg-green-400 text-green-1100': color === 'green', 'outline-neutral-400 bg-neutral-300 text-neutral-900': color === 'neutral' }' stack rounded-full size-28 outline='1 offset--1'>
  ...
  <ComboboxRoot v-model:open='isComboboxOpen' open-on-click open-on-focus>
    <ComboboxAnchor relative>
      ...
      <ComboboxCancel v-if='searchQuery.length > 0' as-child translate-y-0 right-0 top-0 absolute z-1>
        <button flex='~ items-center justify-center' size-36 @click='() => { emit('update:query', undefined); emit('update:category', undefined) }'>
